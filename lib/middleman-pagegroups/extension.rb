...
class MiddlemanPageGroups < ::Middleman::Extension
  ...
  def manipulate_resource_list(resources)
    ...
    resources.sort { |a,b| resource_sort_comparator(a,b) }
      ...
      #--------------------------------------------------------
      # Make `page_group` available for each resource. This is
      ...
      if resource.content_type && resource.content_type.start_with?('text/html', 'application/xhtml')
        ...
        # order. If we change the output directory path for items without a
        # sort order, then we need to add a second step.
        if sort_order && strip
